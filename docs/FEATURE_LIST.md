...
| 12 | A specific company's infrastructure keeps being abused for C2, scanning, or data exfil | Company Block |
...
**The edge case.** A specific company's infrastructure is consistently being used for
malware C2 callbacks, port scanning, credential stuffing, or data exfiltration from LAN
hosts. You know the company name — but its IP ranges rotate across dozens of ASNs and
thousands of CIDRs, making manual CIDR rule maintenance impractical.

**The feature.** The **Company Block** module lets you name the company (e.g.
`datacamp limited`, `private layer inc`). At firewall start, libloc resolves the name to
all matching ASNs and expands each ASN to its full IPv4 CIDR list — building a kernel
`hash:net` ipset per company, collected into a single `list:set` parent. Two O(1) REJECT
rules cover CUSTOMFORWARD (LAN→WAN forwarded) and CUSTOMOUTPUT (traffic from the IPFire
box itself). Any LAN host trying to reach that company's infrastructure gets a REJECT
response immediately.

> **Key constraint:** the company name must be entered as a libloc-compatible search
> substring. Special characters (`&`, `.`, `@`) are not allowed in the key — they
> conflict with the ipset naming constraint (`comp_<key>_net`). For "AT&T" use
> key `att`. Always verify with `location search-as <key>` on the IPFire
> shell before adding — substring matching can produce false positives.
