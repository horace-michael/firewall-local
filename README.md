...
| **Company Block** | Blocks forwarded and outbound traffic to all IPv4 networks owned by named companies — resolved via `location search-as` → ASNs → CIDRs at firewall start |
...
/var/ipfire/fw.local/             # Add-on root (nobody:nobody 755)
  ...
  lists/                          # Master lists (nobody:nobody 644)
    ...
    company_master.list           # Company block (key=STATE,DESC)
...
/srv/web/ipfire/cgi-bin/
  ...
  company-block.cgi               # Company Block
...
| `company_master.list` | `key=STATE,DESC` | key passed to `location search-as`; letters/digits/spaces/hyphens only |
